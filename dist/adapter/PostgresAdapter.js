...
class PostgresAdapter extends BaseAdapter_1.BaseAdapter {
  ...
  async _cloneSchema(tenant) {
    ...
    const sql = `SELECT clone_schema('` + defaultSchema + `', '` + tenant + `', '0');`
  ...
  async _createDropSchemaFunction() {
    ...
    try {
      var sql = fs_1.default.readFileSync(path_1.default.join(__dirname, './sql/drop_schema.sql')).toString()
  ...
  async _createCloneSchemaFunction() {
    ...
    await client.query(`SET search_path TO ${defaultSchema};`)
    try {
      var sql = fs_1.default.readFileSync(path_1.default.join(__dirname, './sql/clone_schema.sql')).toString()
      sql = sql.replace('postgres', credentials.user)
      await client.query(sql)
  ...
  async getViewDefinition(viewName) {
    var _a, _b, _c
    const credentials = this.options.service.credentials
    const schema = (_a = this.options.migrations.schema) === null || _a === void 0 ? void 0 : _a.default
    const query =
    ...
    const viewDefinition = {
      name: viewName,
      definition:
        (_c = (_b = rows[0]) === null || _b === void 0 ? void 0 : _b.view_definition) === null || _c === void 0
          ? void 0
          : _c.replace(regex, ''),
    }
  ...
  async _synchronizeCloneDatabase(schema) {
    const credentials = this.options.service.credentials
  ...
  async _createDatabase() {
    ...
    try {
      //incluimos en el try la conexion
      await client.connect()
