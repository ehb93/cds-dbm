...
class BaseAdapter {
    ...
    async deploy({ autoUndeploy = false, loadMode = null, dryRun = false, createDb = false }) {
        ...
        if (createDb) {
            await this._createDatabase(); //cambio kike no hace falta crear la bbdd porque nos la da ya btp pero para evitar que pete en local lo que hacemos es no lanzar excepcion
                                    //ver postgreeAdapter.js linea 267
        ...
        if (this.options.migrations.multitenant) {
            ...
            for (let i = 0; i < ((_a = this.options.migrations.schema) === null || _a === void 0 ? void 0 : _a.tenants.length); i++) {
                try {
                    const tenant = (_b = this.options.migrations.schema) === null || _b === void 0 ? void 0 : _b.tenants[i];
                    const found = existingSchemas.find((schema) => schema.schema_name === tenant);
                    liquibaseOptions.defaultSchemaName = tenant;
                    if (found) {
                        // Update Tenant Schema
                        await (0, liquibase_1.default)(liquibaseOptions).run(updateCmd);
